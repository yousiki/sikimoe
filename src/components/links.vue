...
<template>
    <div>
            ...
            <div v-for="(card, index) in links" class="card-hover">
                <a :href="card.siteURL">
                    <div class="relative overflow-hidden border-b-2 border-r-2 border-main">
                        ...
                        />
                        <img v-else class="absolute h-full" :src="card.icon" :id="'avatar' + index" :key="index" :alt="'avatar' + card.name" />
        ...
        <p class="text-xs text-right">
            📧:
            <a class="link" href="mailto:you.siki@outlook.com"> you.siki@outlook.com </a>
        </p>
    </div>
</template>
